...
frame iPhone draft editor implemented. Temporal polygon generation and the
explicit sampled approval gate are also implemented; see
[Temporal Mask Generation Contract](mask_generation_contract.md).
...
Authoring uses stable first, quartile, middle, three-quarter, and last frame
indices with duplicates removed for short captures. Generated-mask review uses
a separate, bounded set of up to five frames: it prioritizes low-confidence or
large-change evidence and unseen intermediate frames. Reviewing only the same
five authored polygons would not check their propagation.
...
full-frame propagation and review have succeeded. The current generator
interpolates polygons; it does not track or recognize the subject in the
photos. Capture-time **Limit Reconstruction Area** is a separate screen-fixed
polygon, not a tracked 3D selection.
