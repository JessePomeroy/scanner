...
def preview(run: Path, unit: str) -> dict:
    ...
    is_retry = plan.get('resume_profile') == PROFILE
    ...
    if not is_retry:
        ...
        if len(texture) != 1 or texture[0].get('command') != command(source, source):
    ...
    return {'source_run': str(run), 'source_unit': unit, 'input_workspace': str(source),
            ...
            'runner_sha256': sha256(Path(__file__)), 'images': len(images)}
...
def execute(attempt: Path):
    with attempt_lock(attempt.parent, blocking=True):
        ...
        try:
            ...
            state['stages'] = [{'name': 'texture_mesh', 'status': 'succeeded' if result.returncode == 0 else 'failed',
                                'started_at': state['started_at'], 'finished_at': now(), 'return_code': result.returncode}]
            if result.returncode:
